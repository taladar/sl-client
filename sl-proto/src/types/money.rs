//! Signed L$ values — balances and signed transaction amounts.
...
/// Some wire fields are, however, legitimately *signed* — a group's current
/// balance can be negative, a group-accounting transaction is a positive credit
/// or a negative debit, a refund is a delta either way. Those were carried as
/// raw `i32`, which both lost the L$ meaning and let a balance and an amount mix
/// freely with land areas, counts, and every other `i32` on the wire.
///
...
#[cfg(test)]
mod tests {
    use super::{LindenBalance, NegativeBalanceError};
