...
impl Circuit {
    ...
    /// Queues an `AssetUploadRequest` reliably (the legacy asset store path). The
    /// simulator computes the stored asset id as `combine(transaction_id,
    /// secure_session_id)`. `asset_data` carries the whole asset inline when it
    /// fits a single packet; pass an empty slice to trigger the simulator's
    /// `RequestXfer` (matched by `VFileID`) for a larger asset.
