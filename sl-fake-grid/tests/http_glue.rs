...
mod test {
    ...
    /// `options` list: asking for `voice-config` and getting nothing back is
    /// therefore the grid's answer, not the option filter's.
    #[tokio::test]
    async fn an_opensim_flavoured_login_names_no_voice_backend() -> Result<(), TestError> {
