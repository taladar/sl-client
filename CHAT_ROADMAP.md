...
- [x] **A11. Define the test & verification strategy.** Plan the
  ...
  **Done — see § Test & verification reference (from A11) + task B11 in
  ...
  **survive** (B9). Bidirectional round-trips go in `sim_session.rs` via its
  ...
  reflects it. Runtime **parity** (B10) is tested in the crate command-dispatch
  ...
  **bevy-borrow / tokio-pull** (A10, user-decided). The **only** still-open
  cases are **A12 (voice)** and **A13 (file logs)**, whose tests append to B11
...
- [x] **A12. Design the per-session voice-channel state (signalling only).** A
  ...
  **Done — see § Per-session voice-channel reference (from A12) + task B12 in
  ...
  **Sources:** `has_voice` / `channel` from A5 voice **invite body** plus the
  ...
  gaps — B5 classifies `InviteChannel`, B12 decodes the channel); `joined` set
...
- [x] **A13. Design optional local chat-log files (read + write, all text
  ...
  **Done — see § Local chat-log files reference (from A13) + task B13 in
  ...
  **Read-back reconciled with A10 (the key correction):** Firestorm seeds the
  ...
  the file the **deep archive**, so **B13 serves the *older* pages of
  ...
  A8/B8's history-entry type `ChatMessage` **collides** with the existing
  nearby-chat `ChatMessage` (`types/chat.rs:254`), so it is **renamed
  `SessionMessage`** — threaded through B8 (the entry), A10/B10
  (`ChatHistoryPage { messages: Arc<[SessionMessage]> }`) and B13. **Boundary:**
...
> the last three), and the implementation tasks **B1–B13** below are no longer a
> draft. Phase B **may begin** — one task at a time, in dependency
> order (B1/B2 the registry first), keeping `sl-client-tokio` / `sl-client-bevy`
> / the REPL at parity. **Ask the user before starting Phase B** (the standing
> "ask before new roadmap work" rule). A consolidation pass may still merge
> overlapping inbound-handler tasks (B4 / B6 / B8 share sites) per the
> `PERMISSION_ROADMAP.md` precedent.
...
`Arc<[SessionMessage]>` throughout B10/B13.)
...
| B6 roster/typing | `improved_instant_message_typing_surfaces_im_typing` (`:1263`), the participant arms | `SessionAdd`/`SessionLeave` insert/remove + open session; `participants(Direct)` = `{peer}`; `ImTyping` start/stop sets/clears; 1:1 typing keys by `from_agent_id`; typing does **not** open a session; entry expires after `poll` passes `TYPING_TIMEOUT`; `TypingStop` clears now |
| B7 presence-reset | new (seed roster + 1:1 typing for a friend) | `OfflineNotification` → friend gone from roster + `typing`, **but** sessions still exist + `is_online` false; a non-friend is untouched (only `SessionLeave` removes them); `FriendsOnline` re-adds, changes no session |
| B8 history/unread | `improved_instant_message_surfaces_event` (`:1235`), `read_offline_msgs_caps_surfaces_offline_ims` (`:15048`) | inbound 1:1 logs `unread==1`; group `SessionSend` logs to the group session; own outbound logs (`sender=self`) + resets `unread`; `mark_session_read` resets; `HISTORY_CAP + 1` drops oldest; an `offline==true` IM logs its wire `timestamp` + bumps `unread`; `total_unread` sums |
| B9 persistence | **inverse of** `teleport_clears_seat` (`:1716`) | seed session + history + roster + presence, drive `begin_handover` / `promote_child_to_root` / `TeleportLocal` / child `DisableSimulator` → **all unchanged**; `LogoutReply` keeps them readable on the closed session + a fresh `Session::new` is empty |
| B10 exposure | crate command-dispatch tests | `QueryChatSessions` → light list newest-first, `Invited` flattened; `QueryChatHistoryPage` → bounded newest-first page + `prev`, paging walks older windows without materialising the whole history; `QueryFriends` → right `online` flag; replies are `Arc<[…]>` (clone the `Arc`, compare pointers = no deep copy); a bevy-direct read of `chat_sessions_info()` matches the tokio query reply (data parity) |
...
**Runtime parity (B10).** The three runtimes are checked with the established
...
| **Voice-channel cases** | **STILL OPEN** — A12 not yet designed; its tests append to B11 | A12 |
| **Local chat-log file cases** | **STILL OPEN** — A13 not yet designed; its tests append to B11 | A13 |
...
land and their cases are folded into B11.

### B11. Cross-cutting chat/presence test suite (from A11)

Build the verification suite once B1–B10 exist; it is the integration layer over
the per-task unit tests each B-task already lists (not a replacement for them):

- In `sl-proto/tests/lifecycle.rs`, add the per-task state assertions from the
  case map above, each extending the named existing event test (reuse
  `established` / `inbound_im` / `server_message` / `drain*`). Group them by
  concern (registry, presence, lifecycle, invites, roster/typing,
  presence-reset, history/unread).
- Add the **persistence** test as the explicit inverse of `teleport_clears_seat`
  (`:1716`): seed `chat_sessions` (session + history + a group roster + a typing
  entry) and `friends` / `online`, drive each of the four region-boundary sites,
  and assert every chat/presence datum is **unchanged**; then a `LogoutReply`
  keeps them readable on the closed session and a fresh `Session::new` is empty
  (the B9 keep-for-inspection model).
- Add the **IM-after-offline invariant** test (deliver an IM after an
  `OfflineNotification`; assert the peer stays offline) — the guard for A3's
  presence invariant.
- In `sl-proto/tests/sim_session.rs`, add the bidirectional round-trips (sim
  sends IM / notification / `ChatterBoxInvitation` via `deliver_caps`; client
  store reflects), modelled on `friendship_and_calling_cards_reach_client`.
- In the runtime crates, add the B10 parity checks: the `Arc`-share /
  no-deep-copy assertion and the bevy-direct vs tokio-query data-parity check,
  plus command-dispatch coverage for `AcceptChatInvite` / `DeclineChatInvite`
  / `MarkSessionRead` / the changed `AcceptFriendship`.
- Every test builds, is clippy-clean (restriction lints), and `cargo test`
  passes before its box is ticked.

This task stays **drafted/blocked** until Phase A is signed off; it depends on
all of B1–B10 and is **extended** by the A12 (voice) and A13 (chat-log file)
cases, folded in when those items are designed.
...
| **Per-session** channel (`VoiceChannelState` on `ChatSession`) | the group / conference / 1:1 *room's* voice | **no — A12 adds it** | A12 / B12 |
...
|-------|--------|---------|
| `has_voice` / `channel` | the A5 voice **invite body** (`ChatterBoxInvitation` `voice` body) and the `ChatSessionRequest "accept invitation"` reply `voice_channel_info` | **gaps:** B5 classifies `InviteChannel`; **B12 decodes the channel** (the invitation decoder ignores `voice` today — `conversions.rs:2521`) |
...
| `members` | the modern `ChatterBoxSessionAgentListUpdates` agent-list **voice** flag (the voice-connected subset) | **gap:** B12 decodes the agent-list voice flag — **not** the `is_now_speaking` flag (out of scope) |
...
### B12. Per-session voice-channel state (signalling only) (from A12)

Add the per-session voice facet and the join/leave-voice surface, reusing the
existing voice feature. Lands after B5 (the `ChatSessionRequest` cap + the
`InviteChannel` classification) and B10 (`ChatSessionInfo`):

- Add `VoiceChannelState` + `VoiceChannelInfo` (client-local in sl-proto;
  `Default`-able) and a `voice: VoiceChannelState` field on `ChatSession`;
  `ChatSession::new` starts it empty (no voice).
- **Decode the gaps:** extend `chatterbox_invitation_from_llsd`
  (`conversions.rs:2521`) to read the `voice` body into `has_voice` / `channel`
  (shared with B5's `InviteChannel`); decode the `ChatSessionRequest "accept
  invitation"` reply `voice_channel_info` into `VoiceChannelInfo`; decode the
  modern `ChatterBoxSessionAgentListUpdates` agent-list **voice** flag into
  `members` — **not** the `is_now_speaking` flag.
- Add `Command::JoinSessionVoice { session }` / `LeaveSessionVoice { session }`
  and `Session::join_session_voice` / `leave_session_voice` setting
  `voice.joined` (optimistic). Wire the **runtime** orchestration in
  `sl-client-tokio` / `sl-client-bevy` / the REPL at parity: join = ensure
  ...
  `post_voice_cap` / `post_chat_session_request` (B5), no new HTTP helper.
- Add the accessors `session_has_voice` / `session_voice_channel` /
  `session_voice_joined` / `session_voice_members`; add the voice fields to
  `ChatSessionInfo` (B10) and have `chat_sessions_info()` populate them.
- Fold `voice.members` into A7's offline fan-out (drop an offlined friend from
  the voice membership too) and A9's persistence (voice state survives a
  teleport, clears on logout).
- Tests (SL-only behaviour noted; unit-test decoders on canned LLSD): a voice
  `ConferenceInvited` sets `has_voice` + `channel` (`Voice`/`Both`,
  cross-ref B5); the accept-reply `voice_channel_info` populates
  `VoiceChannelInfo`; `JoinSessionVoice` / `LeaveSessionVoice` flip
  `session_voice_joined`; an agent-list voice update folds `members` (and the
  speaking flag is **ignored**); a 1:1 P2P voice session carries
  `{ self, peer }` membership; `OfflineNotification` drops a friend from
  `voice.members`; a teleport preserves the voice facet.

This task stays **drafted/blocked** until Phase A is signed off; it builds on B5
(the cap + invite classification), B6 (the roster it subsets), B10
(`ChatSessionInfo`), and reuses the existing voice-signalling feature; its tests
fold into B11, closing the A11 voice-cases gap.
...
- **B13 serves the *older* pages of `QueryChatHistoryPage` from file** — when
...
- **timestamp format** — timestamp on/off, date on/off, **seconds on/off
  (default ON)**, 24h/12h;
- **recall window** (default `LOG_RECALL_SIZE` 20480 bytes) — the page size B13
  reads from the file;
...
**Naming correction (cross-cutting, owned here).** A8/B8's planned history-entry
...
…). A13 renames the A8 entry **`SessionMessage`**; it threads through B8 (the
field type), A10 / B10 (`ChatHistoryPage { messages: Arc<[SessionMessage]> }`,
the snapshot builders) and B13 (the parse target). Cross-references added at the
...
### B13. Optional local chat-log files (runtime, all text chat) (from A13)

Implement the runtime file-logging feature in `sl-client-tokio` /
`sl-client-bevy` / the REPL at parity (no sans-IO change beyond the
`SessionMessage` rename). Lands after B8 (the entry type) and B10 (the
`QueryChatHistoryPage` handler it extends):

- Apply the **`ChatMessage` → `SessionMessage`** rename across B8 / A10's view /
  B10 first (the collision fix), then build A13 on it.
- Add a runtime `chat_log` module: a `ChatLogConfig` (default off; the toggles
  above, **seconds default on**), filename construction (`cleanFileName`
  sanitisation, four schemes, optional date suffix, the conference MD5 hash),
  and a writer that appends the Firestorm line
  `[YYYY/MM/DD HH:MM:SS]  Name: message` (local wall-clock, multi-line
...
- Tap the event stream + own-outbound commands: on `ChatReceived` /
  ...
  and on our `InstantMessage` / `SendGroupMessage` / `SendConferenceMessage`,
  write the line to the right file when that type's logging is enabled.
- Extend the B10 **`QueryChatHistoryPage`** runtime handler: when `prev` points
  past the in-memory tail, read the file window (`seek` / `mmap`, the
  `LOG_RECALL_SIZE` page), parse lines back into `SessionMessage`s (failed parse
  → plain-text fallback; fold space-prefixed continuations), and return the page
  plus the older `prev` cursor.
- Optional `conversation.log` index: write/update an entry per conversation,
  purge entries older than the retention days on load.
- Wire `ChatLogConfig` through each runtime's constructor at parity; the REPL
  exposes the toggles.
- Tests (grid-agnostic, run anywhere): a logged message writes the exact
  Firestorm line **with seconds**; each conversation type maps to the right
  sanitised filename (incl. the conference MD5 + the legacy-name option); a
  multi-line message round-trips through `\n␠` continuation; read-back parses
  a stored line into a `SessionMessage` and a malformed line into a plain-text
  fallback; `QueryChatHistoryPage` past the in-memory tail returns file-backed
  pages; the `conversation.log` line format round-trips; default-off writes
  nothing.

This task stays **drafted/blocked** until Phase A is signed off; it is the
runtime long-term store behind A8's in-memory working set and A10's paging,
builds on B8 / B10, and — as the **last** Phase A item's task — completes the
Phase B draft. With A13 done, **Phase A (A1–A13) is complete and signs off**;
Phase B implementation may begin (ask the user first).
