...
`ChatterBox*` session events belong
to `CHAT_ROADMAP.md`). These are now catalogued and batched under the new **CAPS
EventQueue gap** section of [`MISSING_ROADMAP.md`](MISSING_ROADMAP.md);
`AgentStateUpdate` is EQ batch 1, which closes this issue.
