...
/// button, and both were taken — the settings editors' close guard was written
/// and then left unwired because "refusing a close needs the floater chrome to
/// support vetoing one, which it does not"
/// (`viewer-settings-save-as-create-then-put`), and the asset editors carried
/// their own confirmation on a second, near-identical guard component until this
/// one absorbed it.
///
    ...
    for command in messages.read() {
        ...
        match command.op {
            ...
            FloaterOp::Close | FloaterOp::CloseNow => {
                ...
                // question (`CloseNow`) is not asked again. Nothing is touched
...
mod tests {
    ...
    fn a_dirty_window_is_asked_about_before_it_closes() -> Result<(), TestError> {
        ...
        app.update();
        command(&mut app, editor, FloaterOp::BringToFront);
