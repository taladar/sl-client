...
impl GroupPickerState {
    ...
    /// list in).
    ///
    /// The model leads its own list with a [`GroupChoice::NoGroup`] row, and
    /// that one is dropped here in favour of the picker's: the two disagree
    /// about when it belongs. The model suppresses it for an agent who is in no
    /// group, because *its* list is about leaving one and there is nothing to
    /// leave; a picker asking "which group?" still has to offer **none** when
    /// the open said none is an answer — a parcel set to no group is a setting,
        ...
        none.into_iter()
            .chain(
                groups
                    .ordered()
                    .into_iter()
                    .filter(|row| row.group != GroupChoice::NoGroup)
                    .map(|row| GroupPickerRow {
                        group: row.group.key(),
                        name: row.name,
                        detail: String::new(),
                        active: row.active,
...
struct GroupPickOut<'w> {
    /// The answer — the picked group — sent to whoever asked.
    picked: MessageWriter<'w, GroupPicked>,
...
fn spawn_picker_button(
    ...
    let button = ui_spawn::spawn_button(
        commands,
        parent,
        ButtonSpec::bordered(UiLabel::key(label_key), format!("group-picker:{label_key}"))
            .tab_index(tab_index)
            .padding(10.0, 3.0)
...
mod tests {
    ...
    /// A picker that allows **none** offers it **once**, at the top.
    ///
    /// The membership model leads its own list with a `GroupChoice::NoGroup`
    /// row, and the picker prepends one of its own; passing the model's through
    /// as an ordinary group would offer the same answer twice, one row of which
    /// carries a name the picker never localises.
    #[test]
    fn allowing_none_offers_it_once_at_the_top() {
        let state = picker(true);
        assert_eq!(
            state
                .rows
                .iter()
            ...
            vec![None, Some(group(2)), Some(group(1)), Some(group(3))],
            "the \"none\" row leads, and appears exactly once"
        );
        assert_eq!(
            state.rows.iter().map(|row| row.active).collect::<Vec<_>>(),
            vec![false, true, false, false],
            "an agent wearing a group is not wearing none"
        );
    }

    /// The "none" row is drawn as the worn one when the agent wears no group,
    /// and is offered even to an agent who is in no group at all — the model
    /// suppresses its own row there because leaving nothing is meaningless, but
    /// *choosing* none is still an answer (a parcel set to no group).
    #[test]
    fn none_is_offered_and_marked_for_an_agent_wearing_no_group() {
        let mut state = GroupPickerState {
            allow_none: true,
            ..GroupPickerState::default()
        };
        ...
        assert_eq!(
            ...
            "wearing no group marks the none row, and nothing else is worn"
        );

        let mut empty = GroupPickerState {
            allow_none: true,
            ..GroupPickerState::default()
        };
        let rows = empty.membership_rows(&GroupsModel::default());
        empty.set_rows(rows);
        assert_eq!(
            empty.rows.iter().map(|row| row.group).collect::<Vec<_>>(),
            vec![None],
            "an agent in no groups is still offered none"
        );
    }
