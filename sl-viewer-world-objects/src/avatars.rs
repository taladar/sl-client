...
fn spawn_body(
    ...
    // pick — `sl_viewer_world_view::gpu_pick` — picks the drawn, GPU-posed pixels
...
/// (`drive_avatar_motion`) so a same-frame
...
/// `sit_camera_pose`, whose system mutably borrows the camera
/// transform (a seat object is never the camera either).
pub type SeatChainQuery<'world, 'state> = Query<
    'world,
    'state,
...
/// `drive_avatar_motion`) and placed here
...
/// produce.
#[must_use]
pub fn seat_world_transform(seat: Entity, chain: &SeatChainQuery) -> Option<Transform> {
...
pub fn apply_avatar_names(mut events: MessageReader<SlEvent>, mut state: ResMut<AvatarState>) {
    for event in events.read() {
        match &event.0 {
...
/// carries the day).
pub fn flush_name_requests(mut state: ResMut<AvatarState>, mut commands: MessageWriter<SlCommand>) {
...
pub struct RefetchAvatarTextures {
...
/// not decoded yet.
#[derive(Debug, Resource, Default)]
pub struct AvatarBakeMaterials {
...
pub struct AvatarRuntimeMorphs {
...
impl AvatarRuntimeMorphs {
    ...
    pub fn set(&mut self, agent: AgentKey, param: &str, weight: f32) {
    ...
    /// them.
    pub fn clear(&mut self, agent: AgentKey, param: &str) {
