...
pub(crate) fn build_srgb_image(decoded: &Arc<DecodedTexture>) -> Image {
    let mut image = Image::new(
        Extent3d {
            width: decoded.width,
            height: decoded.height,
...
/// what lets [`crate::render_scene`]'s legacy-material scene exercise the real
...
pub(crate) fn apply_legacy_scalars(
    material_asset: &mut FaceMaterial,
...
pub(crate) fn preview_legacy_material(
    manager: &mut LegacyMaterialManager,
    textures: &mut TextureManager,
    materials: &mut Assets<FaceMaterial>,
    handle: &Handle<FaceMaterial>,
