...
//!
//! Inserting a new [`Mesh`](bevy::mesh::Mesh) into `Assets<Mesh>` schedules a GPU
...
/// Default per-frame cap on new [`Mesh`](bevy::mesh::Mesh) inserts, shared across
...
#[derive(Resource)]
pub(crate) struct MeshUploadBudget {
...
pub(crate) fn reset_mesh_upload_budget(mut budget: ResMut<MeshUploadBudget>) {
    budget.reset();
}
