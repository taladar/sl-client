...
- [x] `teleport-failed` — provoke a failed teleport; assert `TeleportFailed`.
  ...
  `AlertInfo`).
- [x] `teleport-cross-region` — cross-region teleport; assert the
...
- [x] `teleport-offer-accept` — offer a lure, peer accepts. `2av`. **Green on
...
- [x] `asset-fetch-http` — HTTP `ViewerAsset` fetch through the caching
  ...
  failure.)
- [x] `asset-upload` — upload an asset via CAPS `NewFileAgentInventory`
...
- [x] `notecard-create-update` — create an empty notecard
...
- [x] `wearables-request` — request current wearables (`AgentWearablesRequest`)
  ...
  `[both]`.
- [ ] `current-outfit-folder` — read the Current Outfit Folder (the modern
  ...
  *links* back to the worn wearables/attachments; the case locates that folder
  and fetches its descendents (AIS3 `FetchInventoryDescendents` on SL, UDP
  `RequestFolderContents` on OpenSim, as in `inventory-tree-crawl`) and asserts
  the outfit links resolve — including the mandatory body parts the legacy
  `AgentWearablesUpdate` omits on modern SL. Expected `complete` on aditi (where
  `wearables-request` is only `partial`), the mirror grid-divergence. `[both]`.
