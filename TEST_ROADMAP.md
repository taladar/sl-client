...
- [ ] `notecard-create-update` — create an empty notecard
  ...
  (`Command::UpdateInventoryAsset` with [`AssetType::Notecard`]).
  ...
  SL does not), so this covers the portable path on both. Assert the created
  item, the `AssetUploaded` from the update naming the new body asset, and
  (best-effort) re-fetch the body to confirm it round-trips.
