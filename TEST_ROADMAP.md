...
- [x] `map-blocks-items` — map blocks/items/layer. `1av`. **Green on OpenSim.**
      ...
      **4** regions (the multi-region teleport-test set), the item reply the
      ...
      still arrive).
- [x] `terrain-composition` — read the region's terrain detail assets and their
...
- [x] `wearables-request` — request current wearables (`AgentWearablesRequest`)
  ...
  `[both]`.
- [x] `current-outfit-folder` — read the Current Outfit Folder (the
...
- [x] `set-appearance` — publish appearance (`AgentSetAppearance`) and query the
...
- [x] `attach-detach` — rez attachment, then detach into inventory. `1av`.
...
- [x] `animation-play-stop` — play and stop an animation. `1av`. A viewer
