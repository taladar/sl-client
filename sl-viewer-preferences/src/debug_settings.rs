...
/// The debug settings floater's [`FloaterSpec`] — shared with the `FLOATERS`
/// registry, so the swept window is the one the viewer spawns.
#[must_use]
pub fn debug_settings_floater_spec() -> FloaterSpec {
    FloaterSpec {
...
fn spawn_debug_settings_floater(mut commands: Commands, root: Res<UiRoot>) {
    let handle = spawn_floater(&mut commands, root.0, debug_settings_floater_spec());
    commands
