...
mod tests {
    ...
    /// **Editing one scattering profile materialises only that one.** A frame
    /// that carries no profiles is the common case (every legacy WindLight sky
    /// is one), and a term written into it has to bring its four siblings with
    /// it or it would be stored beside four zeroes. What it must *not* do is
    /// invent the other two profiles: those are keys the asset never had, and
    /// writing them would change a document the user only touched one slider
    /// of.
    #[test]
