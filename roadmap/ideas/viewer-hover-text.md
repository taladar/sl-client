...
This is the same world-anchored-text machinery as [[viewer-name-tags]] —
billboarded, size-clamped, occlusion-aware text over a world entity — so it
should reuse whatever that task lands rather than growing a second text path.

Reference (Firestorm, read-only): `llhudtext`, `LLVOVolume::updateText`.

Deps: [[viewer-name-tags]] (shares the world-anchored text renderer).
