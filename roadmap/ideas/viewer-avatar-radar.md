...
resolution with [[viewer-name-tags]] — display name plus username, legacy
fallback — so it should consume both rather than re-deriving either. Range
matters: coarse locations cover the region and its neighbours, while full
...
[[viewer-social-panels]] and the track sets a beam via [[viewer-beacons]].

Reference (Firestorm, read-only): `fsradar`, `fsfloaterradar`.

Deps: [[viewer-minimap]] (shared nearby-avatar tracking and panel).
