...
It is plumbing, not content — but it is plumbing Firestorm builds, uploads,
attaches and version-manages entirely on its own, and every bridge-backed
feature ([[viewer-fs-bridge-protocol]]) is dead without it.
...
- **How it is made.** The viewer rezzes a prim in-world, uploads the bridge
  ...
  the `#LSL Bridge` folder, and attaches it to **attachment point 31** —
  `HUD Center 2`, a HUD point, so it is invisible and out of the way (nice
  interaction with the Phase 35 work: a HUD attachment we already classify and
  route). It also cleans up the rezzed prim / older versions ("Landscaping"
  container folder, `FSLSLBridge::cleanUpBridgeFolder`).
- **Version management.** On attach it reads the object's name / the version the
  ...
  it. Older `v1.x` / lower-minor copies are detached and removed
  (`FS_MAX_MINOR_VERSION`, the `for` loops over major/minor in
  `fslslbridge.cpp`).
- **Failure modes to respect.** Rez permission denied on the parcel (it needs
  ...
  `@detach` restriction ([[viewer-rlva-enforcement]] — Firestorm keeps
...
to inventory — so this is mostly *orchestration plus a state machine*, and it is
a strong end-to-end exercise of them. It should also be usable head**less**: a
bot on `sl-client` wanting bridge-backed data (avatar Z-offsets, script info)
needs the same lifecycle without a window.

Sensibly split when picked up: (1) detect an existing, current bridge and adopt
it; (2) create one when absent; (3) version-upgrade / repair; (4) the settings
and diagnostics around it. Do (1) first — it is the common case and it is what
makes the protocol task testable.
