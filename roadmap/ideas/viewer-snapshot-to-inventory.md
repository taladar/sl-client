...
blocked_by: [viewer-snapshot-tools, viewer-image-upload]
---
...
floater ([[viewer-snapshot-tools]]) rather than folded into it.
...
That upload is the general texture path — this task feeds
[[viewer-image-upload]] with the snapshot as its source, rather than growing a
second uploader. Scope here is the snapshot-specific part: the power-of-two /
≤1024 constraint and its live-preview feedback, name / description / target
folder, the L$ cost confirmation, and the choice of a permanent upload vs. a
**temporary local texture** (preview it on a prim without paying, a common
...
Deps: [[viewer-snapshot-tools]] (the floater and the captured image),
