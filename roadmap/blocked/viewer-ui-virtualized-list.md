...
entity per row, so a 10k-item inventory ([[viewer-inventory-ui]]) would mean 10k
taffy nodes; recycle a small window of row entities as the viewport scrolls
instead. This is DIY and is **the main technical unknown** of the UI cluster —
no prior art at that scale in Bevy — so it is isolated in its own task.
