...
On regeneration it then reflows ``INDEX.md`` to 80 columns with ``rumdl fmt``
(if ``rumdl`` is on ``PATH``) so the output passes the commit hook's MD013
check without a separate manual step.
...
def main() -> int:
    ...
    index_path.write_text(render_index(tasks), encoding="utf-8")
    print(f"wrote INDEX.md ({len(tasks)} task(s))")
    reflow_index(index_path)
    return 0


def reflow_index(index_path: Path) -> None:
    """Wrap the generated INDEX.md to the 80-column MD013 limit with ``rumdl``.

    The task links are long single lines that trip the ``rumdl`` MD013 check the
    commit hook runs; ``rumdl fmt`` (with ``reflow = true`` in ``rumdl.toml``)
    wraps them. Doing it here means a plain ``python3 roadmap/index.py`` leaves a
    committable file — the reflow can't be forgotten. A missing ``rumdl`` is only
    a warning: the file is still valid, just unwrapped."""
    rumdl = shutil.which("rumdl")
    if rumdl is None:
        print(
            "warning: rumdl not found on PATH; INDEX.md left unwrapped "
            "(run `rumdl fmt roadmap/INDEX.md` before committing)",
            file=sys.stderr,
        )
        return
    result = subprocess.run(
        [rumdl, "fmt", str(index_path)],
        capture_output=True,
        text=True,
        check=False,
    )
    if result.returncode != 0:
        print(
            f"warning: `rumdl fmt` exited {result.returncode}; "
            f"INDEX.md may be unwrapped:\n{result.stderr.strip()}",
            file=sys.stderr,
        )
    else:
        print("reflowed INDEX.md to 80 columns with rumdl")
