...
([[viewer-preferences-floater]]), and verify with a headless A/B: absolute
camera pose plus the screenshot harness, with a per-effect disable env toggle in
the style of the existing `SL_VIEWER_DISABLE_*` switches. Reproducible captures
want [[viewer-screenshot-wait-for-quiescence]] first, otherwise the two frames
differ for reasons that have nothing to do with AO.
