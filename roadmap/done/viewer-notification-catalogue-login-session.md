---
id: viewer-notification-catalogue-login-session
title: Notification catalogue — login & session entries
topic: viewer
status: done
...
Port the **login & session** notifications from the reference
...
generic server-alert and shared-confirm entries. These cover login, MOTD, TOS
agreement, connection and maintenance.

The exact entry set is the **41** rows tagged `family:login-session` with
