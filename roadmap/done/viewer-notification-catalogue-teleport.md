---
id: viewer-notification-catalogue-teleport
title: Notification catalogue — teleport entries
topic: viewer
status: done
...
Port the **teleport** notifications from the reference `notifications.xml` into
...
confirm entries. These cover teleport offers, failures, progress, home and
lures.

The exact entry set is the **75** rows tagged `family:teleport` with
...
Ported as part of the consolidated remaining-families port (77
...
The reference `offer`-type teleport offer / request variants
map to the notify channel (the bespoke offers task owns the real
offer panel).
