---
id: viewer-notification-catalogue-money-economy
title: Notification catalogue — money / economy entries
topic: viewer
status: done
...
Port the **money / economy** notifications from the reference
...
generic server-alert and shared-confirm entries. These cover L$ pay / buy /
upload-cost / insufficient-funds / gift flows.

The exact entry set is the **50** rows tagged `family:money-economy` with
...
Ported as part of the consolidated remaining-families port (48
...
`ConfirmPurchasePassword` (reference `password` kind) maps
to a modal with the shared input field — the field is UNMASKED; the
money flow must add masking before raising it (loud comment at the
entry). `FirstBalanceIncrease` / `FirstBalanceDecrease` are commented
out in the reference and re-tagged `excluded` in the TSV.
