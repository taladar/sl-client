...
- [ ] New `Permissions` bitflags type (the SL `PERM_*` set:
  MODIFY/COPY/TRANSFER/MOVE/…). Replace the five raw `u32` masks
  (`base_mask`/`owner_mask`/`group_mask`/`everyone_mask`/`next_owner_mask`) in
  `ObjectProperties` (`types/object.rs:641-649`), `InventoryItem`
  (`types/inventory.rs:60-68`), and `ObjectOwnershipData`
  (`types/editing.rs:662-670`). Consider a `Permissions5 { base, owner, group,
  everyone, next_owner }` grouping struct so the five can't be transposed at
  construction.
