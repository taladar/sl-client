...
- [x] `ParticleSystem.texture_id`/`target_id` → `Option<TextureKey>`/
  ...
  B part 2")
- [x] **Remaining nil-key fields** (apply the same nil ⇄ `None`
...
- [x] **Verify-then-fix leads** (read to separate real masking from defensive/
...
- [ ] Convert plain `String` URL fields to `url::Url` from the `url` crate
      (use `url::Url` directly — do NOT invent a client-local or `sl-types` URL
      wrapper). Fields whose empty string means "absent" become
      ...
      any other `String` field documented as a URL.
