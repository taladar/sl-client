...
An interactive, tokio-driven Second Life / OpenSim REPL test client. It logs in
as an avatar from a TOML credentials file, drives a live
[`sl-client-tokio`](../sl-client-tokio) session, and lets you type grid commands
at a prompt — every command, event, and wire diagnostic is recorded to an
always-on trace log so a session can be inspected or replayed afterwards.
...
handling; this crate is the thin tokio binary that wires those to a real socket.
...
to obtain a one-time token. Acquisition waits out the tail of the current
wall-clock-aligned 30-second TOTP window when fewer than
