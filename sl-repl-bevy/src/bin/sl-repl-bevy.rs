...
/// How often the headless Bevy schedule runner ticks the session.
...
pub struct RunArgs {
    ...
    #[clap(long, default_value = "sl-repl-bevy.log")]
    log_file: PathBuf,
...
/// The REPL driver's per-run state, kept as a Bevy resource.
#[derive(Resource, Debug)]
struct ReplState {
    /// The placeholder context, seeded from login facts and updated from events.
    ctx: SessionContext,
    /// Whether to fire the read-only smoke battery on the region handshake.
    smoke: bool,
    /// Whether the smoke battery has already been fired this run.
    smoke_fired: bool,
    /// The logged-in avatar's agent id, once known.
    self_agent: Option<Uuid>,
    /// Whether the input channel is still open (cleared once it disconnects).
    input_open: bool,
}

/// The line-input channel, kept as a Bevy resource.
#[derive(Resource, Debug)]
struct LineInput {
    /// Receives interactive or replayed input lines.
    rx: Receiver<String>,
}

/// The optional session transcript recorder (`!Sync`, so a non-send resource).
#[derive(Debug)]
struct Recorder {
    /// The recorder, present only in interactive mode with `--script-out`.
...
fn spawn_line_editor(
    ...
    printer_tx: Sender<Box<dyn ExternalPrinter + Send>>,
) -> std::thread::JoinHandle<()> {
    std::thread::spawn(move || {
        let mut editor = match DefaultEditor::new() {
            Ok(editor) => editor,
            Err(error) => {
                error!("could not create the line editor: {error}");
                return;
            }
        };
        match editor.create_external_printer() {
            ...
            Err(error) => {
                error!("could not create the external printer: {error}");
                return;
            }
        }
        loop {
...
fn feed_replay(lines: Vec<String>, line_tx: Sender<String>) {
    for raw in lines {
...
fn init_logging(log_file: &Path, terminal: TerminalSink) -> Result<(), Error> {
    ...
    let file_name = log_file
        .file_name()
        .map_or_else(|| OsString::from("sl-repl-bevy.log"), OsString::from);
...
fn handle_input(raw: &str, ctx: &mut SessionContext, commands: &mut EventWriter<SlCommand>) {
    match parse_line(raw) {
        ...
        Ok(Some(ReplAction::Command(pending))) => match pending.resolve(&*ctx) {
            Ok(command) => {
                info!("{}", format_command(&command, &*ctx));
                commands.write(SlCommand(command));
            }
            Err(error) => warn!("could not build command: {error}"),
        },
...
fn repl_driver(
    ...
    for event in events.read() {
        state.ctx.apply_event(&event.0);
        info!("{}", format_event(&event.0, &state.ctx));
        match &event.0 {
            ...
            SlSessionEvent::LoggedOut | SlSessionEvent::Disconnected(_) => {
    ...
    for diagnostic in diagnostics.read() {
        warn!("{}", format_diagnostic(&diagnostic.0));
    }
    ...
    loop {
        match line_input.rx.try_recv() {
            Ok(raw) => {
                if let Some(active) = recorder.inner.as_mut()
                    && let Err(error) = active.record(&raw, Instant::now())
                {
                    warn!("could not record line: {error}");
                }
                handle_input(&raw, &mut state.ctx, &mut commands);
            }
...
/// Run one interactive (or replayed) REPL session end-to-end, restarting the
/// Bevy app once per MFA challenge with the acquired token folded in.
///
/// # Errors
///
/// Returns an [`enum@Error`] if credentials cannot be loaded, the log cannot be
/// initialized, or an MFA challenge cannot be answered.
fn run_repl(args: RunArgs) -> Result<(), Error> {
    ...
    if interactive {
        let (printer_tx, printer_rx) = unbounded();
        let _editor = spawn_line_editor(line_tx.clone(), printer_tx);
        let printer = printer_rx.recv().map_err(|_ignored| Error::LineEditor)?;
        init_logging(
            &args.log_file,
    ...
    if let Some(lines) = replay_lines(&mode)? {
        ...
        let _feeder = std::thread::spawn(move || feed_replay(lines, line_tx));
    }
    drop(line_tx);
...
fn main() -> Result<(), Error> {
    ...
    match options.command {
        ...
        Some(Subcommand::GenerateShellCompletion { output_file, shell }) => {
            ...
            clap_complete::generate(shell, &mut command, "sl-repl-bevy", &mut file);
        }
    }
    Ok(())
}
